...
for (i in 1:13){
  ...
  # Set values for X
  xbin = 0:max(dat.merged.multiple$smokePM_pred)
...
# Save results
results_dir <- paste0("/n/dominici_nsaph_l3/Lab/projects/wildfires-hospitalizations-glm/model_run/results/feols_splines/knts",kts[1],"_",kts[2],"_",kts[3],"/all_lags/clusterSE/")
ifelse(!dir.exists(results_dir), dir.create(results_dir, recursive=TRUE), FALSE)
save(results, file = paste0(results_dir,'results_CM_full.RData'))
...
# Plot results
plot_dir <- paste0("/n/dominici_nsaph_l3/Lab/projects/wildfires-hospitalizations-glm/model_run/figures/feols_splines/knts",kts[1],"_",kts[2],"_",kts[3],"/all_lags/clusterSE/")
ifelse(!dir.exists(plot_dir), dir.create(plot_dir, recursive=TRUE), FALSE)

pdf(paste0(plot_dir,'results_CM_full.pdf'),paper='special',width=15,height=20)
...
# reorder CCS level 1 causes for plotting
combined_results$facet = factor(combined_results$facet,
                                levels=c('Cardiovascular diseases','Respiratory diseases','Cancers','Injuries','Neuropsychiatric disorders',
                                         'Blood diseases','Digestive system diseases','Endocrine disorders','Genitourinary diseases',
                                         'Infectious and parasitic diseases','Musculoskeletal and connective tissue diseases',
                                         'Nervous system diseases','Skin and subcutaneous tissue diseases', 'All'))
...
combined_results %>%
  ...
  geom_ribbon( aes(x = as.numeric(xbin), ymin = lower, ymax = upper, fill = cause), alpha = 0.2) +
  geom_hline(yintercept = 0, color = "darkred", linetype = "dashed") +  
  labs(x = expression(paste("Smoke PM"[2.5], " Concentration (", mu, "g/m^3)")), y = "Change in Unscheduled Hospitalizations per 100k") +
