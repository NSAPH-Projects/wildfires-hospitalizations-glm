...
# The exposure data is subset to non-extreme values
# The confidence intervals used are uniform confidence intervals
...
#################################################################################
## Function to compute derivative estimates and CIs (for testing nonlinearity) ##
#################################################################################
derivative_analysis <- function(model, xbin, knots, vcov_matrix) {
  delta <- 1  # Small step size
  
  # Compute boundary knots
  bk <- attr(ns(xbin, knots = knots), "Boundary.knots")
  
  # Compute natural splines for x and x + delta
  spline_list_x <- list()
  spline_list_x_plus_delta <- list()
  for (lag in 0:7) {
    var_name <- paste0("smokePM_l", lag)
    
    # Apply natural splines with the same knots
    spline_x_terms <- as.data.frame(ns(xbin, knots = kts, Boundary.knots = bk))
    spline_x_plus_delta_terms <- as.data.frame(ns(xbin + delta, knots = kts, Boundary.knots = bk))
    
    # Rename to match the model's expected names
    colnames(spline_x_terms) <- paste0("ns(", var_name, ", ", knot_labels, ")", seq_len(ncol(spline_x_terms)))
    colnames(spline_x_plus_delta_terms) <- paste0("ns(", var_name, ", ", knot_labels, ")", seq_len(ncol(spline_x_plus_delta_terms)))
    
    # Store in list
    spline_list_x[[lag + 1]] <- spline_x_terms
    spline_list_x_plus_delta[[lag + 1]] <- spline_x_plus_delta_terms
  }
  # Combine all splines into a single matrix
  spline_x <- do.call(cbind, spline_list_x)
  spline_x_plus_delta <- do.call(cbind, spline_list_x_plus_delta)
  
  
  # Compute derivative estimates
  derivative_estimates <- (spline_x_plus_delta - spline_x) / delta
  
  # Extract relevant coefficients
  beta_coeff <- coef(model)[1:ncol(derivative_estimates)]
  
  # Compute derivative values
  derivative_values <- as.matrix(derivative_estimates) %*% beta_coeff
  
  # Compute standard errors
  se_deriv <- sqrt(diag(as.matrix(derivative_estimates) %*% vcov_matrix %*% t(as.matrix(derivative_estimates))))
  
  # Compute confidence intervals
  upr <- derivative_values + 1.96 * se_deriv
  lwr <- derivative_values - 1.96 * se_deriv
  
  return(data.frame(xbin = xbin, derivative = derivative_values, lower = lwr, upper = upr))
}
...
# Compute derivative estimates for each cause ( for testing nonlinearity)
all_derivatives <- list() 

autocorr_p <- data.frame()
...
for (i in 1:13){
  ...
  hosp <- sum(dat.merged.multiple$cases)

  total_hosp <- sum(total_hosp,hosp)
  ...
  ## Define person months ##
  # summing the populations for each county-day then dividing by the number of days in a wildfire season (183 days)
  sum(dat.merged.multiple$population)/183
  ...
  # Get AIC
  AIC_results <- AIC(reg_results)
  
  # Test for autocorrelation
  # Extract Residuals
  pdata$residuals <- residuals(reg_results)
  
  # Autocorrelation Test
  n <- length(pdata$residuals)
  auto_mod <- lm(pdata$residuals[-n] ~ pdata$residuals[-1])
  summary(auto_mod)  # Print summary of autocorrelation test
  
  # Check for significant autocorrelation
  if (summary(auto_mod)$coefficients[2, 4] < 0.05) {
    cat("Significant autocorrelation detected in residuals. Consider further adjustments to the model.")
  } else {
    cat("No significant autocorrelation detected.")
  }
  
  # Save p-value
  p <- summary(auto_mod)$coefficients[2, 4]
  autocorr_p <- rbind(autocorr_p, cbind(cause = cause_group, p_val = p))
  ...
  # Save results
  results <- rbind(results, cbind(cause = cause_group, AIC = AIC, xbin = xbin, est = response_curve_b, SE = se.fit, uprP = response_curve_b + (2 * se.fit),
                                  lwrP = response_curve_b - (2 * se.fit),
                                  uprS = response_curve_b + (crit * se.fit),
                                  lwrS = response_curve_b - (crit * se.fit)))
  
  # Compute derivative estimates (for testing nonlinearity)
  deriv_results <- derivative_analysis(reg_results, xbin, kts, model_vcov)
  deriv_results$cause <- cause_group
  all_derivatives[[i]] <- deriv_results
...
colnames(results) <- c("cause ","AIC","xbin","est", "SE", "uprP", "lwrP", "uprS","lwrS")
...
# Save results
results_dir <- paste0("/n/dominici_nsaph_l3/Lab/projects/wildfires-hospitalizations-glm/model_run/results/feols_splines/knts",kts[1],"_",kts[2],"_",kts[3],"/all_lags/clusterSE/nonExtreme_subset/")
ifelse(!dir.exists(results_dir), dir.create(results_dir, recursive=TRUE), FALSE)
save(results, file = paste0(results_dir,'results_CM_unifCI.RData'))
save(autocorr_p, file = paste0(results_dir,'results_autocorr.RData'))
...
# Plot results
plot_dir <- paste0("/n/dominici_nsaph_l3/Lab/projects/wildfires-hospitalizations-glm/model_run/figures/feols_splines/knts",kts[1],"_",kts[2],"_",kts[3],"/all_lags/clusterSE/nonExtreme_subset/unifCI/")
ifelse(!dir.exists(plot_dir), dir.create(plot_dir, recursive=TRUE), FALSE)

pdf(paste0(plot_dir,'results_CM_temp_unifCI.pdf'),paper='special',width=15,height=20)
...
############################
## Save plots with same y ##
############################

pdf(paste0(plot_dir,'results_CM_temp_unifCI_samey.pdf'),paper='special',width=15,height=20)
...
####################################################
## Figure 3 in Paper: Plot subset of main results ##
####################################################
pdf(paste0(plot_dir,'results_CM_temp_unifCI_subset.pdf'),paper='special',width=12,height=10)

# Reorder facet variable
combined_results_subset<- combined_results %>% filter(facet %in% c('Cardiovascular diseases',
                                                                          'Respiratory diseases',
                                                                          'Injuries','Neuropsychiatric disorders',
                                                                          'Digestive system diseases',
                                                                          'Endocrine disorders'))

# Plot the data
combined_results_subset %>%
  ggplot() +
  geom_line(aes(x = as.numeric(xbin), y = as.numeric(est), color = cause)) +
  geom_ribbon(aes(x = as.numeric(xbin), ymin = as.numeric(lower), ymax = as.numeric(upper), fill = cause), alpha = 0.2) +
  geom_hline(yintercept = 0, color = "darkred", linetype = "dashed") +  
...
##############################
## 8. Test for nonlinearity ##
##############################
derivative_results <- do.call(rbind, all_derivatives)

# Rename causes CCS level 1
derivative_results$cause = gsub('Diseases of the circulatory system', 'Cardiovascular diseases', derivative_results$cause)
derivative_results$cause = gsub('Diseases of the respiratory system', 'Respiratory diseases', derivative_results$cause)

derivative_results$cause = gsub('Neoplasms', 'Cancers', derivative_results$cause)

derivative_results$cause = gsub('Injury and poisoning', 'Injuries', derivative_results$cause)
derivative_results$cause = gsub('Mental illness', 'Neuropsychiatric disorders', derivative_results$cause)

derivative_results$cause = gsub('Diseases of the blood and blood-forming organs', 'Blood diseases', derivative_results$cause)
derivative_results$cause = gsub('Diseases of the digestive system', 'Digestive system diseases', derivative_results$cause)
derivative_results$cause = gsub("Endocrine; nutritional; and metabolic diseases and immunity disorders", 'Endocrine disorders', derivative_results$cause)
derivative_results$cause = gsub('Diseases of the genitourinary system', 'Genitourinary diseases', derivative_results$cause)
derivative_results$cause = gsub('Infectious and parasitic diseases', 'Infectious and parasitic diseases', derivative_results$cause)
derivative_results$cause = gsub('Diseases of the musculoskeletal system and connective tissue', 'Musculoskeletal and connective tissue diseases', results$cause)
derivative_results$cause = gsub('Diseases of the nervous system and sense organs', 'Nervous system diseases', derivative_results$cause)
derivative_results$cause = gsub('Diseases of the skin and subcutaneous tissue', 'Skin and subcutaneous tissue diseases', derivative_results$cause)

derivative_results$cause = gsub('Congenital anomalies', 'Other', derivative_results$cause)
derivative_results$cause = gsub('Residual codes unclassified all E codes 259 and 260', 'Other', derivative_results$cause)
derivative_results$cause = gsub('Certain conditions originating in the perinatal period', 'Other', derivative_results$cause)
derivative_results$cause = gsub('Complications of pregnancy childbirth and the puerperium', 'Other', derivative_results$cause)
derivative_results$cause = gsub('Symptoms signs and ill-defined conditions and factors influencing health status', 'Other', derivative_results$cause)

# reorder CCS level 1 causes for plotting
derivative_results$cause = factor(derivative_results$cause,
                       levels=c('Cardiovascular diseases','Respiratory diseases','Cancers','Injuries','Neuropsychiatric disorders',
                                'Blood diseases','Digestive system diseases','Endocrine disorders','Genitourinary diseases',
                                'Infectious and parasitic diseases','Musculoskeletal and connective tissue diseases',
                                'Nervous system diseases','Skin and subcutaneous tissue diseases'))
...
derivative_results <- CreateAllFacet(derivative_results,"cause")

# To avoid having CIs for the "All" panel, set lower and upper = derivative
derivative_results[which(derivative_results$facet=="All"),]$upper <- as.numeric(derivative_results[which(derivative_results$facet=="All"),]$derivative)
derivative_results[which(derivative_results$facet=="All"),]$lower <- as.numeric(derivative_results[which(derivative_results$facet=="All"),]$derivative)

# Sort facets alphabetically, keeping "All" last
facet_levels <- sort(unique(derivative_results$facet[derivative_results$facet != "All"]))
facet_levels <- c(facet_levels, "All")  # Ensure "All" is last

# Reorder facet variable
derivative_results$facet <- factor(derivative_results$facet, levels = facet_levels)
...
save(derivative_results, file = paste0(results_dir,'derivative_results.RData'))

###############################
## Plot derivative estimates ##
###############################
pdf("derivative_analysis.pdf", paper='special',width=15,height=20)

ggplot(derivative_results, aes(x = xbin, y = derivative, color = cause)) +
  geom_line() +
  geom_ribbon(aes(x = as.numeric(xbin), ymin = as.numeric(lower), ymax = as.numeric(upper), fill = cause), alpha = 0.2, color = NA)  +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(x = "Smoke PM2.5 Concentration", y = "Derivative of Response Curve", title = "Exposure-Response Curve Derivatives") +
  facet_wrap(vars(facet), ncol = 2, scales = "free_y") +
  scale_color_manual(values = colors) +
  scale_fill_manual(values = colors) +
  theme_bw() + 
...
#######################################################################
## Compute on-average change per 10 µg/m3 increase in wildfire PM2.5 ##
#######################################################################

results_summary <- combined_results %>%
  group_by(facet) %>%
  summarise(mean_change = mean(as.numeric(est)[as.numeric(xbin) %% 10 == 0], na.rm = TRUE),
            lower_CI = mean(as.numeric(lower)[as.numeric(xbin) %% 10 == 0], na.rm = TRUE),
            upper_CI = mean(as.numeric(upper)[as.numeric(xbin) %% 10 == 0], na.rm = TRUE))

print(results_summary)

save(results_summary, file = paste0(results_dir,'results_change_per_10.RData'))
