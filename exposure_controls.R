...
# For some reason the functions take a while to run
# Instead to find controls, run the lines of code in the second section

# Finding control1 function

find_control1 <- function(same_county_wf,county_wf_edit_array, start_year, end_year){
  i <- 1 # initialize i 
  ...
  if( year(control1_min_date) >= start_year){ #checking if the year is within our bounds
    ...
    # while any of the estimates are greater than our threshhold
  ...
  return(control1_dates)
}


# Finding control2 function why do the functions take so long?

find_control2 <- function(same_county_wf,county_wf_edit_array, start_year, end_year){
  i <- 1 # initialize i 
  ...
  # Specity control2 to be the year before exposed year
  year(control2_min_date) <- year(min_date_temp) + i
  year(control2_max_date) <- year(max_date_temp) + i
  
  
  if( year(control1_min_date) <= end_year){ #checking if the year is within our bounds
    # Check if the control days have a PM2.5 level < threshold
...
# Save control bounds
save(control_dates_df, file = "~/Documents/Harvard/NSAPH Wilfires/control_dates_2006_2018_thresh1.RData")
