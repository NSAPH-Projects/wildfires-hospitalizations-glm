...
regression_function <- function(data, kts) {
  ...
  xunit="GEOID"
  controls="+ ns(Tmean_C, df = 2) + poverty + holiday_indicator" 
  #kts=c(10,25,40)
...
for (i in 1:13){
  ...
  # how do i do the different predictions?
  ...
  pred <- transform(cbind(data.frame(response_curve_b), new_data_df),
                    uprP = response_curve_b + (2 * se.fit),
...
save(results, file = paste0(results_dir,'results_CM_unifCI_cov.RData'))
...
pdf(paste0(plot_dir,'results_CM_temp_unifCI_cov.pdf'),paper='special',width=15,height=20)
